...
Revision ID: 60a90856c89f
Revises:
Create Date: 2026-07-07 09:41:18.908764

Snapshot figé du schéma validé en Sprint 1 (cf. database/schema/ddl_postgres.sql
et database/schema/modele_donnees.md pour le MCD/MLD). Une migration Alembic est
immuable une fois appliquée : toute évolution ultérieure du schéma doit passer
par une NOUVELLE révision (`alembic revision -m "..."`), pas par l'édition de
cette migration ni, à terme, par l'édition manuelle de ddl_postgres.sql.
"""
...
# Ordre inverse des dépendances FK pour un DROP propre sans avoir besoin de
# CASCADE (CASCADE ajouté quand même par sécurité si l'ordre était modifié).
DOWNGRADE_TABLES = [
    "data_quality_log",
...
def downgrade() -> None:
    """Downgrade schema."""
    op.execute(f"DROP TABLE IF EXISTS {', '.join(DOWNGRADE_TABLES)} CASCADE")
