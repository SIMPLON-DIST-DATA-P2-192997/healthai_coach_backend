...
Script complet, slide par slide, rédigé en phrases à dire à voix haute (pas des puces). Utilisable par n'importe lequel des quatre membres de l'équipe — base commune de préparation, et filet de sécurité si l'un d'entre nous est empêché : chaque partie (Rôle A, Rôle B API avec Florian, Rôle C ETL avec Johane/William) est rédigée avec le même niveau de détail, pour être présentable par quelqu'un d'autre que la personne qui l'a construite.
...
Bonjour à tous. Nous allons vous présenter HealthAI Coach, le backend data engineering d'une plateforme de coaching santé, développé en équipe de quatre dans le cadre de cette formation Simplon : Alexandre Laugier, Florian Abgrall, Johane Decamps et William Mibelli. On va dérouler le contexte, l'architecture, ce qu'on a construit concrètement, une démonstration live, puis un bilan honnête — obstacles compris.
...
Le travail a été réparti en trois rôles. Rôle A, Alexandre : architecture, base de données, interface d'administration, dashboard Metabase et dockerisation. Rôle B, Florian : l'API REST en FastAPI. Rôle C, Johane et William : le pipeline ETL et l'orchestration Airflow. On a adopté un flux de travail strict dès le départ : chaque étape correspond à une Pull Request, avec des tests obligatoires et une revue avant fusion. Ce workflow a payé — plusieurs bugs réels ont été attrapés en revue avant d'être fusionnés. On précise aussi qu'en fin de projet, le rôle A a repris une partie de l'orchestration Airflow, l'équipe ETL n'ayant pas eu le temps matériel de la finaliser dans le calendrier de la formation.
