...
Le schéma relationnel (\texttt{database/schema/ddl\_postgres.sql}) compte 14 tables, réparties en quatre domaines fonctionnels autour de l'entité centrale \texttt{USERS} : nutrition, activité physique, suivi déclaratif de santé, et abonnements. Chaque table porte des commentaires SQL (\texttt{COMMENT ON TABLE/COLUMN}) documentant directement en base la signification de chaque champ — en particulier les conventions non triviales comme les clés d'idempotence \texttt{UNIQUE(source, external\_id)}.

\subsection{Modèle Conceptuel de Données — périmètre OLTP}

La figure~\ref{fig:mcd-core} présente le cœur du modèle : nutrition, activité physique et suivi de santé déclaratif, tous rattachés à \texttt{USERS}. La figure~\ref{fig:mcd-subscriptions} présente le domaine abonnements, ajouté dans un second temps pour l'offre B2C/B2B.
...
\draw[rel] (nutlog.east) -- ++(0.6,0) |- (users.west) node[card, pos=0.05]{(1,1)} node[card, pos=0.95]{(0,N)};
...
\draw[rel] (sess.west) -- ++(-0.6,0) |- (users.east) node[card, pos=0.05]{(1,1)} node[card, pos=0.95]{(0,N)};
\draw[rel] (users) -- (bio) node[card, pos=0.15]{(0,N)} node[card, pos=0.85]{(1,1)};
\draw[rel] (users) -- (med) node[card, pos=0.15]{(0,N)} node[card, pos=0.85]{(1,1)};
\draw[rel] (users) -- (diet) node[card, pos=0.15]{(0,N)} node[card, pos=0.85]{(1,1)};
\draw[rel] (users) -- (fit) node[card, pos=0.15]{(0,N)} node[card, pos=0.85]{(1,1)};
\draw[rel] (users) -- (rec) node[card, pos=0.15]{(0,N)} node[card, pos=0.85]{(1,1)};
\draw[relD] (users) -- (dql) node[card, pos=0.15]{(0,N)} node[card, pos=0.85]{(0,1)};
...
\caption{MCD — cœur du modèle (nutrition, activité physique, suivi de santé). Cardinalités au format Merise (min,max), lues côté entité adjacente.}
\label{fig:mcd-core}
\end{figure}
...
% TODO : tiers free/premium/premium_plus, B2B, statut du volet IA.
...
Instance self-hosted (\texttt{docker compose}, cf. \S7), connectée en lecture à PostgreSQL, pour le suivi des KPI de qualité de données (nombre d'anomalies par sévérité/table, taux de résolution) en complément de l'interface admin — Metabase pour la vue d'ensemble/tendances, l'interface admin pour l'action (résolution au cas par cas).
