...
- **Aucun DAG n'apparaît dans l'UI Airflow (`/home`) alors que `docker exec ... airflow dags list` le trouve bien** : même cause que ci-dessus, sur `airflow/logs/` cette fois. `airflow dags list` parse le fichier directement (donc fonctionne même si le fichier est valide), mais le webserver lit la table `DagModel`, alimentée par le `DagFileProcessorManager` du scheduler — si celui-ci crash-boucle avec `PermissionError: [Errno 13] Permission denied: '/opt/airflow/logs/dag_processor_manager'` (visible dans `docker compose logs airflow-scheduler`), rien n'est jamais synchronisé en base. Fix : `sudo chmod -R 777 airflow/logs && docker compose restart airflow-scheduler`, puis attendre ~15-20s.
